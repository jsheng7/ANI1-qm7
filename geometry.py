...
class Geometry(object):
    ...
    def empirical_formula_str(self, padding = 4):
        ...
        syms = [ELEMENTS[z].symbol for z in ef.keys()]
        nums = [ef[z] for z in ef.keys()]
        # from https://stackoverflow.com/questions
        #25539593/sort-a-list-then-give-the-indexes-of-
        # the-elements-in-their-original-order
        indices = zip(*sorted(enumerate(syms), key=itemgetter(1)))[0]
        if padding == 0:
    ...
    def local_rotation(self, atom_or_bond):
        ...
        if len(atom_or_bond) == 1:
             for conn_order in xrange(1,self.natom ):
                temp = [atom_or_bond[0]]
...
def analyze_geometries(geoms,draw_histogram = False, fig_num = 1):
    ...
    for geom in geoms:
        for k,v in geom.get_dists().iteritems():
            draw[k].extend(v)
    dists = {}
    for k,v in draw.iteritems():
        if len(v) > 0:
...
def random_triatomics(ngeom, Zs, bond_range0, bond_range1, angle_range, 
                      rotate = True):
    bl0 = np.random.uniform(bond_range0[0],bond_range1[1], ngeom)
    bl1 = np.random.uniform(bond_range0[0],bond_range1[1], ngeom)
